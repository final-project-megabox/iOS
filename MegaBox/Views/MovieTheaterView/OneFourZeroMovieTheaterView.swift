//
//  OneFourZeroMovieTheaterView.swift
//  MegaBox
//
...
class OneFourZeroMovieTheaterView: UIView {
  
  var alphbetIndex = 0
  var seatButtonArr: [UIButton] = []
  var selectedSeatLabelArr: [UILabel] = []
  
  var delegate: OneFourZeroMovieTheaterViewDelegate?
  
  let alphbetArr = ["A", "B", "C", "D", "E", "F", "G", "H", "I"]
  ...
  @objc private func touchUpPreviousButton() {
    delegate?.touchUpOneFourZeroPreviousButton()
  }
  
  @objc private func touchUpDismissButton() {
    delegate?.touchUpOneFourZeroDismissButton()
  }
  
  @objc private func touchUpSelectOkButton() {
    delegate?.touchUpOneFourZeroSelectOkButton(seatNumber: selectedSeatArr, seatCount: selectedSeatArr.count)
  }
  
  @objc private func touchUpButton(_ sender: UIButton) {
    delegate?.touchUpOneFourZeroTheaterSeat(sender, seatButtonArr, totalCount: totalCount)
  }
    ...
    for i in 0..<9 {
      for j in 0..<20 {
        ...
        if j == 0 {
          ...
          button.setTitle("\(alphbetArr[alphbetIndex])\(j - 2)", for: .normal)
          button.accessibilityIdentifier = "\(j - 2)"
        }
        
        if i == 0 && (j == 4 || j == 5) {
          button.isHidden = true
        } else if i == 8 && (j == 17 || j == 18) {
          button.isHidden = true
        } else if j == 0 || j == 3 || j == 16 || j == 19 {
          button.isHidden = true
        } else if i == 0 && (j == 4 || j == 5) {
  ...
  private func setUpProperties() {
    ...
    screenImage.widthAnchor.constraint(equalToConstant: 1500).isActive = true
    ...
    firstView.widthAnchor.constraint(equalToConstant: 35 * 20).isActive = true
    ...
    exitImage.leadingAnchor.constraint(equalTo: scrollView.leadingAnchor).isActive = true
    ...
    for i in 0..<seatButtonArr.count {
      if i < 20 {
        ...
        if i % 20 == 0 {
          seatButtonArr[i].topAnchor.constraint(equalTo: seatButtonArr[i - 20].bottomAnchor).isActive = true
          seatButtonArr[i].leadingAnchor.constraint(equalTo: firstView.leadingAnchor).isActive = true
          seatButtonArr[i].widthAnchor.constraint(equalToConstant: 35).isActive = true
          seatButtonArr[i].heightAnchor.constraint(equalToConstant: 35).isActive = true
        } else {
