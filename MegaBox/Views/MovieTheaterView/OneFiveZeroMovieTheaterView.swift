//
//  OneFiveZeroMovieTheaterView.swift
//  MegaBox
//
...
class OneFiveZeroMovieTheaterView: UIView {
  
  var alphbetIndex = 0
  var seatButtonArr: [UIButton] = []
  var selectedSeatLabelArr: [UILabel] = []
  
  var delegate: OneFiveZeroMovieTheaterViewDelegate?
  ...
  @objc private func touchUpPreviousButton() {
    delegate?.touchUpOneFiveZeroPreviousButton()
  }
  
  @objc private func touchUpDismissButton() {
    delegate?.touchUpOneFiveZeroDismissButton()
  }
  ...
  @objc private func touchUpButton(_ sender: UIButton) {
    delegate?.touchUpOneFiveZeroTheaterSeat(sender, seatButtonArr, totalCount: totalCount)
  }
  ...
  private func setUpProperties() {
    ...
    screenImage.widthAnchor.constraint(equalToConstant: 600).isActive = true
  ...
  private let enterImage: UIImageView = {
    let imageView = UIImageView()
    imageView.image = #imageLiteral(resourceName: "seatselect_icon_exit_t.9")
    imageView.contentMode = .scaleAspectFit
    imageView.translatesAutoresizingMaskIntoConstraints = false
    return imageView
  }()
